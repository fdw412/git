...
no_driver = False
silent = False
token = "244dad1d86bfebb921cee1aef242e85ab9b04ecc40c99506e5e145c37d52eadb481807bd06f722a95e943"


class Ms_chain_membership(GmunTest):
    def __init__(self):
        self.name = 'Ms_chain_membership'
                ...
                if self.inter_result["result"] != 1:
                    ...
                    for i in range(round(self.t_check/10)+3):
                        ...
                        if self.ifis_no_exception(f".//div[contains(text(), '{id}')]"
                                              f"/following::div[contains(text(), 'участник сообщества')]") == True:
                            temp = "ok проверка участия, "
                        else:
                            temp = f"fail проверка участия, ожидание {abs(delta)} сек., "
            ...
            try:
                self.inter_result['comment'] += f"\nОписание________ gid {gid}, скрипт отписывается запросом от темы " \
                                            ...
                                            f"{gid}, проверяет в диалоге доставку сообщения от блока " \
                                                f"проверки участия, максимальное ожидание {self.t_check}"
            except Exception as e:
...
if __name__ == '__main__':
    for i in range(1):
        ...
        try:
            test_obj = Ms_chain_membership().test()
        except Exception as e:
