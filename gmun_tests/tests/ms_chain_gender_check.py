...
no_driver = False
silent = False
token = "6ca41de5797fed8018b86914b6881e101f8db12b9d5eeb013be76cb38bf20499bfbade9635d5a49f1905d"


class Ms_chain_gender_check(GmunTest):
    def __init__(self):
        self.name = 'Ms_chain_gender_check'
                    ...
                    try:
                        ...
                        for i in range(round(self.t_check / 10) + 3):
                            ...
                            if self.ifis_no_exception(f".//div[contains(text(), '{id}')]"
                                                  f"/following::div[contains(text(), 'проверка пола прошла')]") == True:
                                temp = "ok проверка пола, "
                            else:
                                temp = f"fail проверка пола, ожидание {abs(delta)} сек., "
            ...
            try:
                self.inter_result['comment'] += f"\nОписание________ gid {gid}, скрипт отписывается запросом от темы " \
                                            ...
                                            f"{gid}, проверяет в диалоге доставку сообщения от блока " \
                                                f"проверки пола, максимальное ожидание {self.t_check}"
            except Exception as e:
...
if __name__ == '__main__':
    for i in range(1):
        ...
        try:
            test_obj = Ms_chain_gender_check().test()
        except Exception as e:
