...
class Ale_And_Triggers(GmunTest):
    def __init__(self):
        self.name = 'Ale_And_Triggers'
    ...
    def test(self, paused=True):
                    ...
                    try:
                        ...
                        for i in range(3+round(self.t_check/10)):
                            ...
                            if self.ifis_no_exception(f".//div[contains(text(), '{id}')]"
                                ...
                                temp = temp + 'ok сообщение от блока с отложенным вызовом пришло, '
                            else:
                                temp = temp + f'fail, сообщение от блока с отложенным вызовом не пришло за {delta} сек., '

                            if self.ifis_no_exception(f".//div[contains(text(), '{id}')]"
                                                            f"/following::div[contains(text(), '1 пришло')]") == True:
                                temp = temp + 'ok триггер сработал, '
                            else:
                                temp = temp + f'fail сообщение блока триггера не дождался за {delta} сек., '

                            if self.ifis_no_exception(f".//div[contains(text(), '{id}')]"
                                                            f"/following::div[contains(text(), '2 не пришло')]") == True:
                                temp = temp + 'ok проверка fallback, '
                            else:
                                temp = temp + f'fail сообщение fallback не дождался за  {delta} сек., '
            ...
            try:
                self.inter_result['comment'] += f"\n\nОписание________ gid {gid}, сценарий 52845, скрипт отписывается запросом от темы " \
                                            ...
                                            f"который должен отправить fallback сообщение, максимальное ожидание {self.t_check}, отписывается от темы"
            except Exception as e:
...
@app.task(soft_time_limit=450, time_limit=460, name='ale_and_triggers')
def ale_and_triggers():
    try:
        Ale_And_Triggers().test(paused=False)
    except Exception as e:
        Mattermost().celery_exception_send(f"ale_and_triggers, {e}")

app.control.purge()
app.conf.beat_schedule = {'ale_and_triggers': {'task': 'ale_and_triggers','schedule': 470, 'options': {'queue': 'ale_and_triggers'}}}

if __name__ == '__main__':
        print("started at ", time.strftime('%H:%M:%S').replace("'", ""))
        try:
            test_obj = Ale_And_Triggers().test()
        except Exception as e:
